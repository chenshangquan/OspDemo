...
namespace DuiLib
{
	CVerticalLayoutUI::CVerticalLayoutUI() : m_iSepHeight(0), m_uButtonState(0), m_bImmMode(false),m_bSliderable(false),m_bLButtonDown(false)
	{
		...
		for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
			...
			if( sz.cy == 0 ) {
				...
				if( iAdjustable == nAdjustables ) {
					sz.cy = MAX(0, szRemaining.cy + rcPadding.top - cyFixedRemaining);
				} 
				if( sz.cy < pControl->GetMinHeight() ) sz.cy = pControl->GetMinHeight();
				if( sz.cy > pControl->GetMaxHeight() ) sz.cy = pControl->GetMaxHeight();
			}
	...
	void CVerticalLayoutUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("slideable")) == 0 ) SetSlideable(_tcscmp(pstrValue, _T("true")) == 0);//设置是否响应鼠标拖动消息
	...
	void CVerticalLayoutUI::DoEvent(TEventUI& event)
	{
		...
		{
			//layout点击拖动实现 dyy 2015-5-15
			if (m_bSliderable == false)
			{
	...
	void CVerticalLayoutUI::SetSlideable( bool bSliderable )
	{
		m_bSliderable = bSliderable;
	}

            ...
            if( _tcscmp(pstrClass, _T("Image")) == 0 ) {
                ...
                for( int i = 0; i < nAttributes; i++ ) {
                    ...
                    if( _tcscmp(pstrName, _T("name")) == 0 ) {
						...
						if ( CPaintManagerUI::IsSelfAdaption() )//字体大小自适应
						{
							float fAdpX, fAdpY;
...
CControlUI* CDialogBuilder::Create( CMarkupNode* pNode,CControlUI* pParent /*= NULL*/, CPaintManagerUI* pManager /*= NULL */,IDialogBuilderCallback* pCallBack /*= NULL*/ )
{
	m_pCallback = pCallBack;
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
		...
		//树控件XML解析
            ...
            case 4:
				...
				else if( _tcscmp(pstrClass, _T("Case")) == 0 )		          pManager->AddCase(&node);//xml中添加“方案”配置,在此创建 2014-12-25
                break;
            case 5:
            ...
            case 9:
				...
				else if( _tcscmp(pstrClass, DUI_CTR_TOUCHLIST) == 0 )         pControl = new CTouchListUI; 
                break;
            case 10:
			...
			case 11:
				...
				else if( _tcscmp(pstrClass, DUI_CTR_LETTERINDEX) == 0 )       pControl = new CLetterIndexUI;
				break;
            case 14:
				...
				else if( _tcscmp(pstrClass, DUI_CTR_SLIDETABLAYOUT) == 0 )    pControl = new CSlideTabLayoutUI;
                break;
            case 15:
		...
		//ASSERT(pControl);当节点类型是Case时pControl为NULL， 2014-12-25

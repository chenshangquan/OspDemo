...
bool CPaintManagerUI::m_bSelfAdaption = false;
float CPaintManagerUI::m_fAdpScaleX = 1.00000;
float CPaintManagerUI::m_fAdpScaleY = 1.00000;
...
void CPaintManagerUI::SetInitSize(int cx, int cy)
{
	...
	if ( CPaintManagerUI::IsSelfAdaption() )
	{
		...
		m_szInitWindowSize.cx = cx * fAdpX;
		m_szInitWindowSize.cy = cy * fAdpY;
	}
	else
	{
		m_szInitWindowSize.cx = cx;
...
void CPaintManagerUI::SetMinInfo(int cx, int cy)
{
	...
	if ( CPaintManagerUI::IsSelfAdaption() )
	{
		float fAdpX, fAdpY;
		CPaintManagerUI::GetAdpResolution( &fAdpX, &fAdpY );
		m_szMinWindow.cx = cx * fAdpX;
		m_szMinWindow.cy = cy * fAdpY;
	}
	else
	{
		m_szMinWindow.cx = cx;
...
void CPaintManagerUI::SetMaxInfo(int cx, int cy)
{
	...
	if ( CPaintManagerUI::IsSelfAdaption() )
	{
		float fAdpX, fAdpY;
		CPaintManagerUI::GetAdpResolution( &fAdpX, &fAdpY );
		m_szMaxWindow.cx = cx * fAdpX;
		m_szMaxWindow.cy = cy * fAdpY;
	}
	else
	{
		m_szMaxWindow.cx = cx;
...
bool CPaintManagerUI::PreMessageHandler(MSG* pMsg)
{
    for( int i = 0; i < m_aPreMessageFilters.GetSize(); i++ ) 
    {
        bool bHandled = false;
		LRESULT lResult = static_cast<IPreMessageFilterUI*>(m_aPreMessageFilters[i])->PreMessageHandler(pMsg);
        if( bHandled ) {
            return true;
        }
    }
...
bool CPaintManagerUI::DefaultMessageHandle(UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& lRes)
{
	...
	case WM_SHOWWINDOW:
	{
						  // 由于layeredwindow响应不到WM_PAINT,在ShowWindow的时候不会重绘
						  // 导致显示出窗口显示隐藏之前的状态（如Button点击图标）
						  // 这里手动刷新一下，代码有待优化 by zjl
						  // domodel窗口按钮弹出式不刷新 第一次显示的时候进行刷新 dyy 2015-5-7
						  if (m_bAlphaBackground && /*!*/m_bFirstLayout && wParam == TRUE)
						  {
							  if (m_pRoot->IsUpdateNeeded() || m_bAlphaBackground)
							  {
								  if (m_hDcOffscreen != NULL) ::DeleteDC(m_hDcOffscreen);
								  if (m_hDcBackground != NULL) ::DeleteDC(m_hDcBackground);
								  if (m_hbmpOffscreen != NULL) ::DeleteObject(m_hbmpOffscreen);
							  ...
							  {
								  // 由于UpdateLayeredWindow不支持局部重绘，所以要重画整个客户区
								  RECT rcClient = { 0 };
								  ::GetClientRect(m_hWndPaint, &rcClient);

								  // 这里获取到的ps.rcPaint永远是(0,0,0,0)
								  PAINTSTRUCT ps = { 0 };
								  ...
								  if (m_bPaintOnWindow)
								  {
									  ...
									  ::UpdateLayeredWindow(m_hWndPaint, NULL, &pt, &szWindow, m_hDcOffscreen, &ptSrc, 0, &blendPixelFunction, ULW_ALPHA);
								  }
	...
	case WM_PAINT:
	{
					 ...
					 PAINTSTRUCT ps = { 0 };
					 ::BeginPaint(m_hWndPaint, &ps);
					 if (m_bOffscreenPaint || !m_bPaintOnWindow)
					 {

						 HBITMAP hOldBitmap = (HBITMAP) ::SelectObject(m_hDcOffscreen, m_hbmpOffscreen);
						 int iSaveDC = ::SaveDC(m_hDcOffscreen);

						 // 将要绘制的区域的颜色清空，以解决透明图片残影问题
						 // 这里用不支持Alpha通道的函数，使Alpha通道变为0 by zjl
						 CRenderEngine::DrawColor(m_hDcOffscreen, ps.rcPaint, 0xFF000000);
...
// 						   TCHAR chTemp[128];
// 						   wsprintf(chTemp,_T("UIEVENT_BUTTONDOWN  ptGetGestury.x:%d,y:%d \n" ),pt.x, pt.y);
...
// 
// 							 TCHAR chTemp[128];
// 							 wsprintf(chTemp,_T("UIEVENT_DBLCLICK  ptGetGestury.x:%d,y:%d \n" ),pt.x, pt.y);
// 							 ::OutputDebugString(chTemp);
	}
	break;
	case WM_GESTURENOTIFY:    //触屏点击 dyy 2015-5-15 
		{
			GESTURENOTIFYSTRUCT* pGestureNty =  (GESTURENOTIFYSTRUCT*)(lParam);   //触摸点参数
			RECT rectWnd;
			::GetWindowRect( m_hWndPaint, &rectWnd );
			
			POINT ptGetGestury;
			ptGetGestury.x = pGestureNty->ptsLocation.x - rectWnd.left;
			ptGetGestury.y = pGestureNty->ptsLocation.y - rectWnd.top;

// 			TCHAR chTemp[128];
// 			wsprintf(chTemp,_T("WM_GESTURENOTIFY  ptGetGestury.x:%d,y:%d pGestureNty->dwInstanceID:%d this:%X \n" ),ptGetGestury.x, ptGetGestury.y,pGestureNty->dwInstanceID,this);
// 			::OutputDebugString(chTemp);
			...
			if ( NULL != pGestureNty )
			{
				...
				POINT pt = { ptGetGestury.x, ptGetGestury.y };
				m_ptLastMousePos = pt;
				CControlUI* pControl = FindControl(pt);    
				if( pControl == NULL ) break;
				if( pControl->GetManager() != this ) break;
				m_pTouchClick = pControl;
				pControl->SetFocus();
				//SetCapture();
				TEventUI event = { 0 };
				event.Type = UIEVENT_GESTURENOTIFY;
				event.pSender = pControl;
				event.wParam = wParam;
				event.lParam = lParam;
				event.ptMouse = pt;
	...
	case WM_RBUTTONUP:
	{
						 ...
						 /*  发送触屏点击消息可能会导致崩溃
						 if (m_pTouchClick != NULL )
						 {
							 TEventUI eventTouch = { 0 };
							 eventTouch.Type = UIEVENT_GESTUREUP;
							 eventTouch.pSender = m_pTouchClick;
							 eventTouch.wParam = wParam;
							 eventTouch.lParam = lParam;
							 eventTouch.ptMouse = pt;
							 eventTouch.wKeyState = (WORD)wParam;
							 eventTouch.dwTimestamp = ::GetTickCount();

							 m_pTouchClick->Event(eventTouch);
							 m_pTouchClick = NULL;
						 }*/					
	}
		break;
...
// 						   TCHAR chTemp[128];
// 						   wsprintf(chTemp,_T("WM_RBUTTONDOWN  ptGetGestury.x:%d,y:%d \n" ),pt.x, pt.y);
// 						   ::OutputDebugString(chTemp);
	}
		break;
	case WM_CONTEXTMENU:
...
bool CPaintManagerUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& lRes)
{
	...
	if (DefaultMessageHandle(uMsg, wParam, lParam, lRes) )
	{
		return true;
	}
...
bool CPaintManagerUI::AddPreMessageFilter(IPreMessageFilterUI* pFilter)
{
    ASSERT(m_aPreMessageFilters.Find(pFilter)<0);
    return m_aPreMessageFilters.Add(pFilter);
}

bool CPaintManagerUI::RemovePreMessageFilter(IPreMessageFilterUI* pFilter)
{
    for( int i = 0; i < m_aPreMessageFilters.GetSize(); i++ ) {
		if (static_cast<IPreMessageFilterUI*>(m_aPreMessageFilters[i]) == pFilter) {
...
const Image* CPaintManagerUI::GetImageP( LPCTSTR strPath )
{
	Image* pImg = static_cast<Image*>(m_mImageName.Find(strPath));
	if( !pImg ) 
	{
		...
		pImg = new Image( strFileFullPath.c_str() ); 
		
		if( !m_mImageName.Insert(strPath, pImg) ) 
		{
			delete pImg;
			return NULL;
		}
...
const TImageInfo* CPaintManagerUI::GetImageEx( LPCTSTR bitmap, LPCTSTR type /*= NULL*/, DWORD mask /*= 0*/,bool bIsAlwaysChangeBk /*= false*/ )
{
	TImageInfo* data = static_cast<TImageInfo*>(m_mImageHash.Find(bitmap));
	if( !data ) {
...
void CPaintManagerUI::RemoveAllImages()
{
	...
	Image* pImg;
	for( int i = 0; i< m_mImageName.GetSize(); i++ ) {
		if(LPCTSTR strPath = m_mImageName.GetAt(i)) {
			pImg = static_cast<Image*>(m_mImageName.Find(strPath));
			if (pImg) {
				delete pImg;
			}
		}
	}
	m_mImageName.RemoveAll();
}
...
// 
// 	IDragController::GetSingletonPtr()->HandleMessage(pMsg);
// 
// 	for( int i = 0; i < m_aPreMessages.GetSize(); i++ ) 
// 	{
// 		CPaintManagerUI* pT = static_cast<CPaintManagerUI*>(m_aPreMessages[i]);  
// 		
// 		HWND hTempParent = pMsg->hwnd;
// 		while (hTempParent)
// 		{
...
{  
	...
	if (uChildRes != 0)  //如果是子窗口，处理消息后返回
	{  
		HWND hWndParent = ::GetParent(pMsg->hwnd);  
		//解决子窗口edit控件按tab无法切换焦点的bug  
...
CPaintManagerUI* CPaintManagerUI::GetManager( HWND hWnd )
{
	for( int i = 0; i < m_aPreMessages.GetSize(); i++ ) {
		CPaintManagerUI* pManager = static_cast<CPaintManagerUI*>(m_aPreMessages[i]);
		if ( pManager != NULL && pManager->GetPaintWindow() == hWnd  )
		{
			return pManager;
		}
	}
...
void CPaintManagerUI::SetPaintOnWindow( bool bPaint )
{
	m_bPaintOnWindow = bPaint;
}

bool CPaintManagerUI::AddCase( CMarkupNode* pNodeRoot )
{
	return m_cCaseManager.AddCase( pNodeRoot, this );
}

bool CPaintManagerUI::DoCase( const LPCTSTR szCaseName, CControlUI *pParent /*= NULL*/ )
{
	return m_cCaseManager.DoCase( szCaseName, this, pParent );
}

//自适应分辨率
void CPaintManagerUI::SetSelfAdaption( bool bAdaption )
{
	m_bSelfAdaption = bAdaption;
}

bool CPaintManagerUI::IsSelfAdaption()
{
	return m_bSelfAdaption;
}

void CPaintManagerUI::SetAdpResolution( SIZE resSrc, SIZE resDst )
{
	m_fAdpScaleX = (float)resDst.cx / resSrc.cx;

	m_fAdpScaleY = (float)resDst.cy / resSrc.cy;
}

void CPaintManagerUI::GetAdpResolution( float* fAdpScaleX, float* fAdpScaleY )
{
	*fAdpScaleX = m_fAdpScaleX;
	*fAdpScaleY = m_fAdpScaleY;
}

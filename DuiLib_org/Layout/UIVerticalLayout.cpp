...
namespace DuiLib
{
	CVerticalLayoutUI::CVerticalLayoutUI() : m_iSepHeight(0), m_uButtonState(0), m_bImmMode(false), m_bDonotify(false)
	{
...
//			cyFixedRemaining -= m_iChildPadding;
	...
	void CVerticalLayoutUI::SetVerticalEvent(bool doevent)
	{
		m_bDonotify = doevent;
	}

	bool CVerticalLayoutUI::IsVerticalEvent() const
	{
		return m_bDonotify;
	}
	...
	void CVerticalLayoutUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("event")) == 0 ) SetVerticalEvent(_tcscmp(pstrValue, _T("true")) == 0);

...
void CMenuUI::SetMaxDrop(int maxdrop)
{
	m_maxdrop = maxdrop;
}

int CMenuUI::GetMaxDrop()
{
	return m_maxdrop;
}
...
{
	if (_tcscmp(pstrName, _T("maxdrop")) == 0)	SetMaxDrop(_ttoi(pstrValue));
...
void CMenuWnd::ResizeMenu()
{
	...
	if (pMenuRoot->GetMaxDrop() != 0 && szInit.cy >= pMenuRoot->GetMaxDrop()) szInit.cy = pMenuRoot->GetMaxDrop();
...
void CMenuElementUI::DoEvent(TEventUI& event)
{
	...
	{
		if (!m_strHotIcon.IsEmpty())
		{
			m_strIcon = m_strHotIcon;
			Invalidate();
		}
	}
	else if( event.Type == UIEVENT_MOUSELEAVE )
	{
		if (!m_strNormalIcon.IsEmpty())
		{
			m_strIcon = m_strNormalIcon;
			Invalidate();
		}
	}
...
void CMenuElementUI::SetHotIcon(LPCTSTR strIcon)
{
	if ( strIcon != _T("") )
		m_strHotIcon = strIcon;
}

void CMenuElementUI::SetNormalIcon(LPCTSTR strIcon)
{
	if ( strIcon != _T("") )
		m_strNormalIcon = strIcon;
}
...
void CMenuElementUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("normalicon")) == 0){
		SetNormalIcon(pstrValue);
	}
